...
class CounterpartyForm(forms.ModelForm):
    ...
    class Meta:
        model = Counterparty
        fields = ['name', 'tin', 'iec', 'comment', 'is_active']
        widgets = {
...
class ContractForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'is_plan': forms.CheckboxInput(
                attrs={
                    'class': 'form-control',
                }),
            'is_border_contract': forms.CheckboxInput(
                attrs={
                    'class': 'form-control',
                }),
            'sum_with_nds': forms.TextInput(
                attrs={
                    'class': 'form-control',
                }),
            'be_nds': forms.CheckboxInput(
                attrs={
                    'class': 'form-control',
                }),
...
class FinPlanFilterForm(forms.Form):
    is_closed = forms.BooleanField(
        required=False,
        label='Показать все договоры, включая закрытые',
...
class PaymentForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if 'counterparty' in self.initial.keys():
            self.fields['contract'].queryset = Contract.objects.filter(
                counterparty=self.initial['counterparty'])
        if 'contract' in self.initial.keys():
            self.fields['payment_stages'].queryset = PaymentStages.objects.filter(
                contract=self.initial['contract'])
    ...
    payment_stages = forms.ModelChoiceField(
        queryset=PaymentStages.objects.all(),
        required=False,
        label='Этап оплаты',
        widget=forms.Select(
            attrs={
