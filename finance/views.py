    ...
    for i in data:
        for i1 in range(1, 16):
            ...
            if i1 < mounth_int:
                ...
                i[key_fact] = i[key_plan]
                i[key_plan] = ''
    return render(request, 'finance/index.html', {'data': data, 'data_sum': data_sum, 'mounth_int': mounth_int, 'title': 'Воронка'})
...
def default_list(request, object):
    ...
    return render(request, 'finance/default_list.html',
                  {'title': get_full_data_about_object(object)['title_list'], 'objects_list': objects_list,
                   'table': create_table(get_full_data_about_object(object)['model'], object, objects_list, titles_list,
                                         fields_list), 'object': object,
                   'button_list': create_button_list(object, request), 'form_filter': form_filter})
...
def default_add(request, object):
        ...
        if 'cancel' in request.POST:
            ...
            if form.is_valid():
                ...
                if 'save_exit' in request.POST:
                    ...
                    return redirect('../default_edit/' + str(object) + '/' + str(obj_.id))
            else:
    ...
    return render(request, 'finance/default_add_edit.html',
                  {'title': get_full_data_about_object(object)['title_add'], 'object': object, 'form': form,
                   'error': error, 'buttons_add_edit': create_button_add_edit(is_new, request, object, 1, None)})


@login_required(redirect_field_name='login')
def default_edit(request, object, id):
    object_from_model = get_object_or_404(get_full_data_about_object(object)['model'], id=id)
    error = ''
    is_new = False
    if request.method == 'POST':
        ...
        if 'delete' in request.POST:
            error = ''
            object_del = get_full_data_about_object(object)['model'].objects.filter(id__exact=id)
            ...
            if parent_model:
                get_full_data_about_object(object)['parent_model'].objects.get(id=parent_id).save()
            if len(error) == 0:
                return redirect(redirect_string)
        elif ('save' in request.POST) or ('save_exit'in request.POST):
            form = get_full_data_about_object(object)['form'](request.POST, instance=object_from_model)
            if form.is_valid():
                ...
                error = 'Форма была заполнена неверно!'
    form = get_full_data_about_object(object)['form'](instance=object_from_model)
    return render(request, 'finance/default_add_edit.html', {
        'title': get_full_data_about_object(object)['title'] + str(get_title_edit(object_from_model, object)) + '"',
        'object': object, 'form': form, 'error': error,
        'buttons_add_edit': create_button_add_edit(is_new, request, object, 2, id, True)})
...
def get_full_data_about_object(object):
    ...
    if object == 'Counterparty':
        ...
        data_list = {'model': PaymentStages,
                     ...
                     'titles_list': ['Название', 'Договор', 'Дата', 'Сумма', 'Сумма с НДС', 'Оплачен', 'Комментарий'],
                     'fields_list': ['title', 'contract', 'date', 'sum', 'sum_with_nds', 'paymented', 'comment'],
...
def create_table(model, object_name, objects_list, titles_list, fields_list):
  ...
  <tbody>'''
    ...
    for object in objects_list:
        i += 1
        link_line = '<a href="../' + link_list + '/' + str(object_name) + '/' + str(object.id) + '">'
        code_line += '''<tr>
  <th scope="row">''' + link_line + str(i) + '</a></th>'
        ...
        for field_from_list in fields_list:
            ...
            if key_field_value == '_no_':
                ...
                code_line += '<td class="text-nowrap">' + link_line + return_result(object_name, field_from_list, object.id) + '</a></td>'
            else:
...
def create_button_list(object, request):
    ...
    code_line = '<a href="../' + link_list + '/' + object + '" class="btn btn-success my-2 float-right" role="button">Добавить (+)</a>'
    return code_line


def copy(request, object, id):
    if object == 'PaymentStages':
        current_object = PaymentStages.objects.get(pk=id)
        current_object.pk = None
        current_object.save()
        return redirect('../../default_edit/' + str(object) + '/' + str(current_object.id))


def create_button_add_edit(is_new, request, object, i, id, delete=False, add_string=False):
    code_line = '<a href = "'
    for i in range(i):
        code_line += '../'
    code_line += get_url_redirect(object, id) + '" class ="btn btn-info my-2" role="button">Отменить</a>'
    if add_string:
        code_line += '''
        <button name = \'add_string\' type = "submit" class ="btn btn-secondary">Добавить строку</button>'''
    if not is_new and object=='PaymentStages':
        code_line += f'''
        <a href = "../../copy/PaymentStages/{id}" class ="btn btn-info my-2" role="button">Копировать</a>'''
    code_line += '''
...
def get_url_redirect(object, id):
    if object == 'PaymentStages' and id:
        object_ps = PaymentStages.objects.get(pk=id)
        # return 'point_edit/Point/' + str(object_point.point.id)
        return 'default_list/' + str(object) + '?contract=' + str(object_ps.contract.id)
    else:
